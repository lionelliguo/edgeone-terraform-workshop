...
Lab directory: `lab-05-domain-management`
...
```bash
cd labs/lab-05-domain-management/terraform
```
...
```bash
cd ../lab-05-domain-management/terraform
```
...
- Complete Terraform output list: `lab_id`, `zone_id`, `acceleration_domain`, `edgeone_cname`, `lab_success`, `created_resource_names`, `current_execution`, `execution_steps`, `next_lab`, `workshop_result`.
...
- `next_lab`: Recommended next Lab after this Lab succeeds. Next Lab: `lab-06-cname-setup`.
- `workshop_result`: Final result summary with `lab_id`, `lab_success`, and `next_lab`.

## Success Criteria

- Terraform creates `tencentcloud_teo_acceleration_domain.www`.
- `acceleration_domain` matches the configured `sub_domain.zone_name`.
- `edgeone_cname` is available for DNS configuration in the next Lab.
