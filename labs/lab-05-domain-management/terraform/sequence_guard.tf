...
resource "terraform_data" "lab_sequence_guard" {
  input = "lab-05-domain-management"

  lifecycle {
    precondition {
      condition     = contains(["lab-04-ownership-verify", "lab-05-domain-management"], local.workshop_previous_lab_id)
      error_message = contains(local.workshop_version_management_lab_ids, local.workshop_previous_lab_id) ? "Version Management has already been enabled by Lab 12 or a later Lab. Running Lab 05 now is rejected to avoid changing earlier immediate-effect configuration after version control is enabled. Continue with the next version-management Lab, or run Lab 16 Resource Cleanup before starting again from Lab 01. Current previous lab_id: ${local.workshop_previous_lab_id}." : "Lab 05 sequence check failed. The shared state was not produced by lab-04-ownership-verify or a previous lab-05-domain-management run. Please start from Lab 01 and run the workshop Labs in order. Expected previous lab_id: lab-04-ownership-verify or lab-05-domain-management. Current Lab: lab-05-domain-management. Use the shared backend state at shared/edgeone-workshop.tfstate, and run terraform init again after switching Lab directories."
    }
  }
}
