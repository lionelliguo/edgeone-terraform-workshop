...
Lab 目录：`lab-05-domain-management`
Lab 名称：Lab 05：域名管理
...
- 完整 Terraform 输出列表：`lab_id`、`zone_id`、`acceleration_domain`、`edgeone_cname`、`lab_success`、`created_resource_names`、`current_execution`、`execution_steps`、`next_lab`、`workshop_result`。
...
- `next_lab`：本 Lab 成功后建议继续执行的下一个 Lab。 下一个 Lab：`lab-06-cname-setup`。
- `workshop_result`：最终结果摘要，固定包含 `lab_id`、`lab_success` 和 `next_lab`。

## 成功标准

- Terraform 创建 `tencentcloud_teo_acceleration_domain.www`。
- `acceleration_domain` 与配置的 `sub_domain.zone_name` 一致。
- `edgeone_cname` 可用于下一个 Lab 的 DNS 配置。
