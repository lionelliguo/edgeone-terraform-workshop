...
resource "tencentcloud_teo_zone" "zone" {
  ...
  depends_on = [time_sleep.before_update_version]
}
...
data "tencentcloud_teo_export_zone_config" "l7_acceleration" {
  ...
  depends_on = [time_sleep.after_update_version_ready]
}
...
locals {
  ...
  update_version_rule_engine_rules_to_add = slice([local.update_version_rule_engine_rule], 0, var.enable_version_management && !local.update_version_rule_engine_rule_exists ? 1 : 0)
...
resource "tencentcloud_teo_deploy_config_group_version" "workshop_l7_staging" {
  ...
  depends_on = [tencentcloud_teo_config_group_version.workshop_l7, time_sleep.after_update_version_ready]
}
...
resource "tencentcloud_teo_deploy_config_group_version" "workshop_l7_update_staging" {
  ...
  config_group_version_infos {
    version_id = tencentcloud_teo_config_group_version.workshop_l7_update[0].version_id
  }
