...
Lab directory: `lab-06-cname-setup`
...
- Complete Terraform output list: `lab_id`, `acceleration_domain`, `edgeone_cname`, `dnspod_cname_record_id`, `lab_success`, `created_resource_names`, `current_execution`, `execution_steps`, `next_lab`, `workshop_result`.
...
- `edgeone_cname`: CNAME assigned by EdgeOne.
- `dnspod_cname_record_id`: DNSPod CNAME record ID; null when DNSPod automation is disabled.
...
- `next_lab`: Recommended next Lab after this Lab succeeds. Next Lab: `lab-07-ssl-certificate`.
- `workshop_result`: Final result summary with `lab_id`, `lab_success`, and `next_lab`.

## Success Criteria

- The business domain has a CNAME path to EdgeOne.
- If `auto_create_dnspod_records = true`, Terraform creates the DNSPod CNAME record.
- If DNS automation is disabled, the output provides the exact CNAME target for manual DNS configuration.
