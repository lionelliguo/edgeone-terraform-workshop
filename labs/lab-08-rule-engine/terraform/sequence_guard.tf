...
resource "terraform_data" "lab_sequence_guard" {
  input = "lab-08-rule-engine"

  lifecycle {
    precondition {
      condition     = contains(["lab-07-ssl-certificate", "lab-08-rule-engine"], local.workshop_previous_lab_id)
      error_message = contains(local.workshop_version_management_lab_ids, local.workshop_previous_lab_id) ? "Version Management has already been enabled by Lab 12 or a later Lab. Running Lab 08 now is rejected to avoid changing earlier immediate-effect configuration after version control is enabled. Continue with the next version-management Lab, or run Lab 16 Resource Cleanup before starting again from Lab 01. Current previous lab_id: ${local.workshop_previous_lab_id}." : "Lab 08 sequence check failed. The shared state was not produced by lab-07-ssl-certificate or a previous lab-08-rule-engine run. Please start from Lab 01 and run the workshop Labs in order. Expected previous lab_id: lab-07-ssl-certificate or lab-08-rule-engine. Current Lab: lab-08-rule-engine. Use the shared backend state at shared/edgeone-workshop.tfstate, and run terraform init again after switching Lab directories."
    }
  }
}
