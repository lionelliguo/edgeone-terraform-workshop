...
- 完整 Terraform 输出列表：`lab_id`、`purge_type`、`purge_job_id`、`purge_targets`、`lab_success`、`created_resource_names`、`current_execution`、`execution_steps`、`next_lab`、`workshop_result`。
- `lab_id`：最近完成并写入共享 state 的 Lab 编号，用于后续 Lab 的顺序保护。
- `purge_type`：缓存清除类型。
- `purge_job_id`：缓存清除任务 Job ID。
- `purge_targets`：缓存清除目标 URL 列表。
...
- `next_lab`：本 Lab 成功后建议继续执行的下一个 Lab。 下一个 Lab：`lab-15-prefetch-urls`。
- `workshop_result`：最终结果摘要，固定包含 `lab_id`、`lab_success` 和 `next_lab`。

## 成功标准

- Terraform 创建 `tencentcloud_teo_purge_task.www_home`。
- `purge_job_id` 有值。
- `purge_targets` 指向当前 `www` 加速域名。
