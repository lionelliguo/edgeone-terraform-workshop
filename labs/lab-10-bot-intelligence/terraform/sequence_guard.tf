...
resource "terraform_data" "lab_sequence_guard" {
  input = "lab-10-bot-intelligence"

  lifecycle {
    precondition {
      condition     = contains(["lab-09-security-policy", "lab-10-bot-intelligence"], local.workshop_previous_lab_id)
      error_message = contains(local.workshop_version_management_lab_ids, local.workshop_previous_lab_id) ? "Version Management has already been enabled by Lab 12 or a later Lab. Running Lab 10 now is rejected to avoid changing earlier immediate-effect configuration after version control is enabled. Continue with the next version-management Lab, or run Lab 16 Resource Cleanup before starting again from Lab 01. Current previous lab_id: ${local.workshop_previous_lab_id}." : "Lab 10 sequence check failed. The shared state was not produced by lab-09-security-policy or a previous lab-10-bot-intelligence run. Please start from Lab 01 and run the workshop Labs in order. Expected previous lab_id: lab-09-security-policy or lab-10-bot-intelligence. Current Lab: lab-10-bot-intelligence. Use the shared backend state at shared/edgeone-workshop.tfstate, and run terraform init again after switching Lab directories."
    }
  }
}
